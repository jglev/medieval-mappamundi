# Script for taking comma-separated strings (within cells of a CSV) and combining them into one-way relationships for easy import into a network mapping application
# Jacob Levernier
# September 2017
# Version: 0.1.0
...
workingDirectoryToSet <- "/path/to/directory/that/contains/Mappamundi_Project_Full_Dataset_Edge_List.csv"
...
fileNameToWriteOutputTo <- "2017-08-27-1404_Mappamundi_Project_Full_Dataset_Edge_List.csv"
...
if (transpose_dataset_first == TRUE) {
	...
	# Remove the first row, which would have been row titles in the originally-transposed dataset:
...
for(i in 1:nrow(dataSetToParse)){
	message("Processing row ", i, "...")
		...
		vector_of_places_in_row_with_blanks_removed <- vector_of_places_in_row[vector_of_places_in_row != ""]
	...
	edge_list_of_places_and_map_names <- rbind(edge_list_of_places_and_map_names, pairwise_combinations_of_places)
} # End of for loop
...
edge_list_with_lowercased_rows.data_frame <- as.data.frame(edge_list_with_lowercased_rows)

unique_values <- as.character(
	sort(unique(
		edge_list_with_lowercased_rows.data_frame[
			grep("Normalized_Reading", edge_list_with_lowercased_rows.data_frame$to, ignore.case = TRUE) # Just use rows from a particular set of maps (ones whose names match the search string here).
			,
			"from"
			]
	))
